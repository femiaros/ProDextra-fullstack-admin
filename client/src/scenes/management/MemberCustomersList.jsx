...
import { useGetMemberCustomersQuery } from "./managementApiSlice"

const MemberCustomersList = () => {
  ...
  let columns = [
    ...
    },
    {
      ...
      width: 100,
      type: "singleSelect",
      valueOptions: ["user","member"],
      sortable: false,
      editable: true,
      renderCell: (params) => {
        if(params.row.roles.includes("member")) return "member"
        return "user"
      }
    },
    {
  ...
  if(!isNonMediumScreens){
    columns = columns.filter(({field})=>{
      return field==='firstName' || field==='company' || field==='country' || field==='active' || field==='avatar' || field==='roles'
    })
  }
  {/* --- Remove Column On Small Screen--- */}
  if(isSmallScreens){
    columns = columns.filter(({field})=>{
      return field==='firstName' || field==='avatar' || field==='roles'
    }).map(item=>{
      if(item.field === 'roles') return{...item,flex: 0.5,width:'auto'}
      return item
    })
  }
  ...
  return (
    <div className="p-4 pb-2 rounded-2xl bg-bgSecondary dark:bg-bgDarkSecondary shadow-card dark:shadow-cardDark">
      {/* Title */}
      <h2 className=' font-semibold'>Member Customers</h2>
...
export default MemberCustomersList